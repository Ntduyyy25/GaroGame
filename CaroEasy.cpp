...
const int AttackArr[4] = {0, 2, 10, 100}; // Điểm tấn công
const int DefendArr[4] = {0, 3, 15, 100}; // Điểm phòng ngự

// Ham kiem tra o co bi chan khong
bool CaroEasy::isBlocked(int x, int y)
{
    return (x < 0 || x >= SIZE || y < 0 || y >= SIZE);
}

int CaroEasy::attackPoint(int x, int y)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (Enemy[i] == 1 && Ally[i] < 3 && Block[i] == 1)
            Point = AttackArr[0];
...
int CaroEasy::defendPoint(int x, int y)
{
    // Các hướng di chuyển chính
    int tX[4] = {1, 0, 1, 1};
    int tY[4] = {0, 1, 1, -1};
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        // Giảm điểm nếu bị chặn
        if ((Ally[i] == 1) || (Block[i] == 1))
            Point /= 2;

        // Không đánh nếu bị chặn ở hai đầu
        if (Ally[i] == 1 && Enemy[i] < 3 && Block[i] == 1)
            Point = DefendArr[0];
...
pair<int, int> CaroEasy::getBestMove()
{
